...
<div class="container">
     ...
     <form action="{{ route('layouts.update', $layout->id) }}" method="POST">
         ...
         <div class="form-check mb-3">
             <input type="checkbox" name="is_active" class="form-check-input" id="isActive" {{ $layout->is_active ? 'checked' : '' }}>
                 ...
                 @foreach ($layout->fields as $index => $field)
                     <tr>
                         <td>
                             ...
                             <input type="text" name="fields[{{ $index }}][field_key]" class="form-control" value="{{ $field->field_key }}" required>
                         </td>
