    ...
    <div class="main" id="main">
        ...
        <form action="{{ route('predefine.update') }}" method="POST">
            ...
            <div class="col-md-3 mt-3">
                ...
                <select id="ExpenseCode" name="ExpenseCode" class="form-control select2" placeholder="Enter Expense Code">
                    <option value="" disabled selected>Choose a Expense Code</option>
                    @foreach($accounts as $account)
                        <option value="{{ $account->HeadCode }}" @if($account->HeadCode == $predefineaccount->ExpenseCode) selected @endif>
                            {{ $account->HeadCode }} - {{ $account->HeadName }}
                        </option>
                    @endforeach
                </select>
