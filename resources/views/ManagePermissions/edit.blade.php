<x-layout>
    ...
    <x-slot name="main">
        <div class="main" id="main">
            <h3>User Name:-{{$managePermission->user->name}}, Department Name:- {{ $managePermission->user->department ? $managePermission->user->department->name : 'ADMIN' }}
            </h3>
                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 col-lg-12">
                            ...
                            <div class="row">
                                ...
                                $d_ids = is_array($managePermission->d_id) ? $managePermission->d_id : json_decode($managePermission->d_id);
                                $p_ids = is_array($managePermission->p_id) ? $managePermission->p_id : json_decode($managePermission->p_id, true);
                                @endphp
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="d_id[]"
                                                value="{{ $department->id }}" id="department_{{ $department->id }}" {{
                                                in_array($department->id, $d_ids) ? 'checked' : '' }}>
                                            ...
                                            @foreach ($permissions as $permission)
                                                <div class="form-check">
                                                    <input type="checkbox" class="form-check-input"
                                                        name="p_id[{{ $department->id }}][]" value="{{ $permission->id }}"
